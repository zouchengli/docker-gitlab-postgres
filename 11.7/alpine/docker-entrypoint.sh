...
docker_create_db_directories() {
	...
	chmod 775 /var/opt/gitlab/postgresql || :
	...
	if [ "$user" = '0' ]; then
		...
		find /var/opt/gitlab/postgresql \! -user postgres -exec chown postgres '{}' +
	fi
}
...
_pg_want_help() {
	...
	for arg; do
		case "$arg" in
			...
			# postgres: could not access the server configuration file "/var/opt/gitlab/postgresql/data/postgresql.conf": No such file or directory
